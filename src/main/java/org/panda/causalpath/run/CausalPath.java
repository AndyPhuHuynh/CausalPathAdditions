...
public class CausalPath
{
	...
	public CausalPath(String directory) throws IOException
	{
		...
		this.logger = new CPLogger(directory);
		readParameters();
	...
	private void setParameter(String key, String value)
	{
		...
		if (param == null)
		{
			logger.paramError.error("Unrecognized parameter: " + key);
			return;
		}
		...
		{
 			logger.paramError.error(key + " requires a numerical input. Input was: '" + value + "'.");
		}
		catch (BooleanFormatException bfe)
		{
			logger.paramError.error(key + " has to be true or false. Input was: '" + value + "'.");
		}
		catch (IllegalValueTransformationException vte)
		{
			logger.paramError.error("Value transformation '" + value + "' not recognized. Possible values are: " + ValueTransformation.getNamesAsString() + ".");
		}
		catch (IllegalThresholdDataFormatException tde)
		{
			logger.paramError.error(tde.getMessage() + " '" + value + "' not recognized. " + tde.getMessage() + " must be in the form of an integer and valid data type separated by a space." +
		...
		{
			logger.paramError.error("Built-in network resource selection '" + value + "' not recognized. Possible values are: " + NetworkLoader.ResourceType.getValuesAsString() + ".");
		}
		catch (IllegalRelationFilterException rfe)
		{
			logger.paramError.error("Relation filter type '" + value + "' not recognized. Possible values are: " + GraphFilter.RelationFilterType.getNamesAsString() + ".");
		}
		catch (IllegalDataTypeException ide)
		{
			logger.paramError.error(ide.getMessage() + " '" + value + "' not recognized. Possible values are: " + ide.getValidValues());
		}
		catch (IllegalGeneActivityLabelException ale)
		{
			logger.paramError.error("gene-activity: '" + value + "' not recognized. Parameter must be in the format of a gene name and activity label " +
		...
		{
			logger.paramError.error(ide.getMessage());
		}
		catch (FileDoesNotExistException fde)
		{
			logger.paramError.error(fde.getMessage());
		}
	}
	...
	private void checkParametersValidity()
	{
		...
		if (proteomicsValuesFile == null) logger.paramError.error("proteomics-values-file is REQUIRED but was not provided");
		if (IDColumn == null) logger.paramError.error("id-column is REQUIRED but was not provided");
		if (symbolsColumn == null) logger.paramError.error("symbols-column is REQUIRED but was not provided");
		if (sitesColumn == null) logger.paramError.error("sites-column is REQUIRED but was not provided");
		if (transformation == null) logger.paramError.error("value-transform is REQUIRED but was not provided");
		if (valueColumn == null && controlValueColumn == null && testValueColumn == null)
			logger.paramError.error("Either value-column OR (control-value-column AND test-value-column) are REQUIRED," +
			"but neither were provided");

		if (valueColumn != null && controlValueColumn != null) logger.paramError.error("value-column and control-value-column are mutually exclusive");
		if (valueColumn != null && testValueColumn != null) logger.paramError.error("value-column and test-value-column are mutually exclusive");

		// Check if given files exist
		if (proteomicsValuesFile != null && !fileExists(adjustFileLocation(proteomicsValuesFile)))
			logger.paramError.error("Provided filepath for proteomics-value-file: '" + proteomicsValuesFile + "' does not exist");
		if (rnaExpressionFile != null && !fileExists(adjustFileLocation(rnaExpressionFile)))
			logger.paramError.error("Provided filepath for rna-expression-file: '" + rnaExpressionFile + "' does not exist");
		if (acetylProteomicFile != null && !fileExists(adjustFileLocation(acetylProteomicFile)))
			logger.paramError.error("Provided filepath for acetyl-proteomic-file: '" + acetylProteomicFile + "' does not exist");
		if (methylProteomicFile != null && !fileExists(adjustFileLocation(methylProteomicFile)))
			logger.paramError.error("Provided filepath for methyl-proteomic-file: '" + methylProteomicFile + "' does not exist");
		if (mutationEffectFilename != null && !fileExists(adjustFileLocation(mutationEffectFilename)))
			logger.paramError.error("Provided filepath for mutation-effect-file: '" + mutationEffectFilename + "' does not exist");
		if (tfActivityFile != null && !fileExists(tfActivityFile))
			logger.paramError.error("Provided filepath for tf-activity-file: '" + tfActivityFile + "' does not exist");
		if (customCausalPriorsFile != null && !fileExists(customCausalPriorsFile))
			logger.paramError.error("Provided filepath for custom-causal-priors-file: '" + customCausalPriorsFile + "' does not exist");
		...
		if (transformation == ValueTransformation.AS_IS_SINGLE_VALUE || transformation == ValueTransformation.SIGNED_P_VALUES)
		{
			if (valueColumn == null || valueColumn.isEmpty())
				logger.paramError.error("value-transformation: '" + transformation.name + "' requires one value column");
		}
		...
		{
			if (valueColumn == null || valueColumn.isEmpty())
				logger.paramError.error("value-transformation: '" + transformation.name + "' requires one or more value columns");
		}
				...
				transformation == ValueTransformation.SIGNIFICANT_CHANGE_OF_MEAN)
		{
			if (controlValueColumn == null || controlValueColumn.isEmpty())
				logger.paramError.error("value-transformation: '" + transformation.name + "' requires one or more control value columns");
			if (testValueColumn == null || testValueColumn.isEmpty())
				logger.paramError.error("value-transformation: '" + transformation.name + "' requires one or more test value columns");
		}
		else if (transformation == ValueTransformation.SIGNIFICANT_CHANGE_OF_MEAN_PAIRED)
		{
			...
			if (controlValueColumn != null && testValueColumn != null && controlValueColumn.size() != testValueColumn.size())
				logger.paramError.error("value-transformation: '" + transformation.name + "' requires an equal amount of control and test" +
		...
		else if (transformation == ValueTransformation.CORRELATION)
		{
			if (valueColumn == null || valueColumn.size() < 3)
				logger.paramError.error("value-transformation: '" + transformation.name + "' requires three or more test value columns");
		}

		// Check threshold-data-for-significance
		if (thresholdForDataSignificance != null && fdrThresholdForDataSignificance != null)
			logger.paramError.error("threshold-for-data-significance and fdr-threshold-for-data-significance are mutually exclusive");

		if (transformation == ValueTransformation.CORRELATION)
		{
			if (thresholdForDataSignificance != null)
				logger.paramWarning.warn("threshold-for-data-significance will be ignored because value-transformation is correlation");
			if (fdrThresholdForDataSignificance != null)
				logger.paramWarning.warn("fdr-threshold-for-data-significance will be ignored because value-transformation is correlation");
		}
		else if (transformation != ValueTransformation.ERROR)
		{
			...
			{
				if (thresholdForDataSignificance == null && fdrThresholdForDataSignificance == null)
					logger.paramError.error("value-transformation: '" + transformation.name + "' requires either threshold-for-data-significance or " +
							"fdr-threshold-for-data-significance");
			}
			else{
				if (thresholdForDataSignificance == null)
					logger.paramError.error("value-transformation: '" + transformation.name + "' requires threshold-for-data-significance");
				if (fdrThresholdForDataSignificance != null)
					logger.paramError.error("value-transformation: '" + transformation.name + "' does not use fdr-threshold-for-data-significance");
			}
		}

		// Check parameters that are only used with correlation
		if (pvalThresholdForCorrelation != -1 && fdrThresholdForCorrelation != -1)
			logger.paramError.error("pval-threshold-for-correlation and fdr-threshold-for-correlation are mutually exclusive");
		if (transformation != ValueTransformation.CORRELATION && transformation != ValueTransformation.ERROR)
		{
			if (correlationValueThreshold != -1)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use correlation-value-threshold. " +
						"correlation-value-threshold will be ignored");
			if (correlationUpperThreshold != null)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use correlation-upper-threshold. " +
						"correlation-upper-threshold will be ignored");
			if (pvalThresholdForCorrelation != -1)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use pval-threshold-for-correlation. " +
						"pval-threshold-for-correlation will be ignored");
			if (fdrThresholdForCorrelation != -1)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use fdr-threshold-for-correlation. " +
		...
		if (!calculateNetworkSignificance)
		{
			if (permutationCount != 1000)
				logger.paramWarning.warn("permutations-for-significance will be ignored because calculate-network-significance is false");
			if (fdrThresholdForNetworkSignificance != 0.1)
				logger.paramWarning.warn("fdr-threshold-for-network-significance will be ignored because calculate-network-significance is false");
			if (useNetworkSignificanceForCausalReasoning)
				logger.paramWarning.warn("use-network-significance-for-causal-reasoning will be ignored because calculate-network-significance is false");
			if (minimumPotentialTargetsToConsiderForDownstreamSignificance != 5)
				logger.paramWarning.warn("minimum-potential-targets-to-consider-for-downstream-significance will be ignored because calculate-network-significance is false");
		}

		if (transformation == ValueTransformation.CORRELATION && cs.getPrioritizeActivityData())
			logger.paramError.error("value-transformation: '" + transformation.name + "' does not use prioritize-activity-data");

		if (!cs.getForceSiteMatching() && cs.getSiteProxitmityThreshold() != 0)
			logger.paramWarning.warn("site-match-proximity-threshold will be ignored because do-site-matching is false");

		// Check parameters not supposed to be used with correlation
		if (transformation == ValueTransformation.CORRELATION)
		{
			if (colorSaturationValue != 1)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use color-saturation-value. " +
						"color-saturation-value will be ignored");
			if (showAllGenesWithProteomicData)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use show-all-genes-with-proteomic-data. " +
						"show-all-genes-with-proteomic-data will be ignored");
			if (showInsignificantData)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use show-insignificant-data. " +
						"show-insignificant-data will be ignored");
			if (hideDataNotPartOfCausalRelations)
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use hide-data-not-part-of-causal-relations. " +
						"hide-data-not-part-of-causal-relations will be ignored");
			if (!activityMap.isEmpty())
				logger.paramWarning.warn("value-transformation: '" + transformation.name + "' does not use gene-activity. " +
...
//		saveRels(testedRels);
	...
	private boolean directoryExists(String filepath)
	{
